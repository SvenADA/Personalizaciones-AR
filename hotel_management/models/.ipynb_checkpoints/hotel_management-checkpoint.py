...
class checkout_configuration(models.Model):
    ...
    company_id = fields.Many2one(
        'res.company', related='shop_id.company_id', string='Compañía', store=True)
...
class sale_order(models.Model):
    ...
    state = fields.Selection(selection_add=[
        ...
        ('done', 'Hecho'),
        ('cancel', 'Cancelado')
    ], string='Estado de pedido', readonly=True, index=True)

    amount_untaxed = fields.Monetary(string='Monto no agravado', store=True, readonly=True, compute='_amount_all',
                                     tracking=5)

    amount_tax = fields.Monetary(
        string='Impuestos', store=True, readonly=True, compute='_amount_all')
...
class hotel_reservation(models.Model):
    ...
    date_order = fields.Datetime('Fecha de reservación', required=True, default=datetime.today(), readonly=True,
                                 states={'draft': [('readonly', False)]})
    shop_id = fields.Many2one('sale.shop', 'Hotel', required=True,
                              default=_get_default_shop, readonly=True, states={'draft': [('readonly', False)]})
    partner_id = fields.Many2one(
        'res.partner', 'Nombre del huésped', required=True, readonly=True, states={'draft': [('readonly', False)]})
    pricelist_id = fields.Many2one(
        'product.pricelist', 'Tarifa de precios', required=True, readonly=True, states={'draft': [('readonly', False)]})
    ...
    childs = fields.Integer('Niños',)
    reservation_line = fields.One2many(
        'hotel.reservation.line', 'line_id', ' reservación', readonly=False, states={'done': [('readonly', True)]})
    state = fields.Selection([('draft', 'Borrador'), ('confirm', 'Confirmado'), (
        'done', 'Hecho'), ('cancel', 'Cancelado')], 'Estado', default='draft', readonly=True)
    folio_id = fields.Many2many(
        'hotel.folio', 'hotel_folio_reservation_rel', 'order_id', 'invoice_id', 'Folio')
    dummy = fields.Datetime('Dummy')
    source = fields.Selection([('internal_reservation', 'Reservación interna'), ('through_web', 'A través del sitio web'), (
        'through_gds', 'A través de GDS')], 'Fuente', default='internal_reservation', index=True, readonly=True,
        states={'draft': [('readonly', False)]})
    number_of_rooms = fields.Integer(
        compute="_count_total_rooms", string="Número de habitaciones")
    untaxed_amt = fields.Float(
        compute="_get_subtotal_amount", string="Importe no gravado")

    total_tax = fields.Float(string="Impuestos de reservación",
                             compute="_get_total_tax")
    deposit_cost1 = fields.Float(string="deposit cost", )
    total_cost1 = fields.Float(
        string="Costo total de reservación", compute="_get_total_rental_cost", readonly=True)
    company_id = fields.Many2one(
        'res.company', related='shop_id.company_id', string='Compañía', store=True)

    id_line_ids = fields.One2many('hotel.resv.id.details', 'reservation_id',
                                  'ID Line', readonly=False, states={'done': [('readonly', True)]})
    via = fields.Selection([('direct', 'Directo'), ('agent', 'Agente')], "Vía",
                           readonly=True, default='direct', states={'draft': [('readonly', False)]}, )
    agent_id = fields.Many2one('res.partner', 'Agente', readonly=True, states={
        'draft': [('readonly', False)]}, )
    invoiced = fields.Boolean('Invoiced', default=False)
    adv_amount = fields.Float("Monto de adelanto")

    deposit_recv_acc = fields.Many2one(
        'account.account', string="Cuenta de depósito", required=False)

    deposit_cost2 = fields.Float(
        compute="_get_advance_cost", string="Anticipo", store=True)
    ...
    total_advance = fields.Float(
        compute="_get_advance_payment", string="Anticipo total", )
    remaining_amt = fields.Float(
        compute="_get_advance_payment", string="Monto total restante", )
    show_update_pricelist = fields.Boolean(string='¿Han cambiado los precios?')

    folio_count = fields.Integer(
        compute="_compute_folio", string='Recuento de folios', copy=False, default=0, readonly=True)
    folio_ids = fields.Many2many('hotel.folio', compute="_compute_folio", string='Facturas', copy=False,
                                 readonly=True)
...
class hotel_reservation_line(models.Model):
    ...
    currency_id = fields.Many2one(
        related='line_id.currency_id', store=True, string='Moneda', readonly=True)
    line_id = fields.Many2one('hotel.reservation')
    room_number = fields.Many2one('product.product', 'Número de habitación ',
                                  domain="[('isroom','=',True),('categ_id','=',categ_id)]",
                                  help="Will list out all the rooms that belong to selected shop.", required=True)
    categ_id = fields.Many2one(
        'product.category', 'Tipo de habitación', required=True, )
    price = fields.Float("Precio")
    discount = fields.Float('Descuento (%)', digits=(16, 2))
    number_of_days = fields.Integer(string="Número de días", default=1)
    sub_total1 = fields.Float(
        string='Sub Total', store=True, readonly=True, compute='count_sub_total1', compute_sudo=False)
    taxes_id = fields.Many2many('account.tax', 'reservation_taxes_rel', 'prod_id', 'tax_id', 'Impuestos', domain=[
        ('type_tax_use', 'in', ['sale', 'all'])])
    checkin = fields.Datetime(
        'Fecha de Checkin', required=True, default=lambda self: fields.Datetime.now())
    checkout = fields.Datetime('Fecha de Checkout', required=True, )
    company_id = fields.Many2one(
        related='line_id.company_id', comodel_name='res.company', string='Compañía', store=True)
...
class hotel_room_booking_history(models.Model):
    ...
    product_id = fields.Integer("ID del producto", readonly=True)
    history_id = fields.Many2one("hotel.room", "Número de habitación", readonly=True)
    name = fields.Char('Nombre del producto', readonly=True, required=True)
    category_id = fields.Many2one(
        "product.category", "Categoría de habitación", readonly=True)
    check_in = fields.Datetime('Fecha de CheckIn', readonly=True)
    check_out = fields.Datetime('Fecha de CheckOut', readonly=True)
    check_in_date = fields.Date('Fecha de Check In', readonly=True)
    check_out_date = fields.Date('Fecha de Check Out', readonly=True)
    partner_id = fields.Many2one('res.partner', "Nombre", readonly=True)
    booking_id = fields.Many2one(
        'hotel.reservation', "Referencia de reservación", readonly=True)
    state = fields.Selection([('draft', 'Borrador'), ('confirm', 'Confirmar'),
                              ('cancle', 'Cancelar'), ('done', 'Hecho')], 'State', readonly=True)
    company_id = fields.Many2one(
...
class hotel_folio(models.Model):
    ...
    total_advance = fields.Float(
        compute="_get_advance_payment", string="Anticipo total", )
    remaining_amt = fields.Float(
        compute="_get_advance_payment", string="Total restante", )

    account_move_ids = fields.Many2many('account.move', 'sale_account_move_rel', 'sale_id', 'move_id',
                                        "Payment Details")
    is_agent_commission_created = fields.Boolean('¿Se crea una comisión de agente?')
    ...
    laundry_line_ids = fields.One2many(
        'hotel_folio_laundry.line', 'folio_id', 'Referencia de Folio')

    reservation_id = fields.Many2one('hotel.reservation', 'Referencia de reserva')
    ...
    rooms_ref = fields.Char(
        compute="_rooms_reference", string="Número de habitación", compute_sudo=False)
    rooms_ref1 = fields.Char(compute="_rooms_reference", compute_sudo=False,
                             string="Número de habitación", store=True)
...
class hotel_restaurant_tables(models.Model):
    ...
    shop_id = fields.Many2one('sale.shop', 'Hotel', states={'confirmed': [('readonly', True)]},
                              help="Mostrará la lista de tiendas que pertenecen a las empresas permitidas del usuario registrado. \n -Asignar el nombre de la tienda a la que no pertenece esta tabla.")
    avl_state = fields.Selection([('available', 'Disponible'), ('book', 'Reservado')], 'Disponibilidad',
                                 default='available', index=True, required=True,
                                 states={'confirmed': [('readonly', True)]})
    state = fields.Selection(selection_add=[('draft', 'Borrador'), ('edit', 'Editar'), ('confirmed', 'Confirmado'), (
        'canceled', 'Cancel')], string='Estado', required=True, readonly=True,
...
class hotel_restaurant_order(models.Model):
    ...
    waiter_name1 = fields.Many2one('res.users', 'Camarero asignado')
    ...
    state = fields.Selection(selection_add=[('draft', 'borrador'), ('confirm', 'Confirmado'), ('order', 'Pedido listo'), (
        'done', 'Hecho'), ('cancel', 'Cancelado')], string='Estado', index=True, required=True, readonly=True)
    flag = fields.Boolean("Flag", default=False)
    amount_subtotal = fields.Float(
        compute="_sub_total", string='SubTotal', compute_sudo=False)
    amount_tax = fields.Float(compute="_amount_tax", string='Impuesto')
    amount_total = fields.Float(compute="_total", string='Total')
    amount_untaxed = fields.Float(
        'Monto sin impuestos', compute='_amount_untaxed', store=True)
...
class hotel_restaurant_reservation(models.Model):
    ...
    pricelist_id = fields.Many2one(
        'product.pricelist', 'Tarifa de precios', required=True)
...
class hotel_restaurant_order_list(models.Model):
    ...
    product_id = fields.Many2one('hotel.menucard', 'Nombre del producto', required=True)
    product_qty = fields.Integer('quantity')
    states = fields.Boolean('States', default=False)
    previous_qty = fields.Integer('quantities')
    total = fields.Integer('total')
    tax_id = fields.Many2many(
        'account.tax', 'restaurant_order_tax', 'order_line_id', 'tax_id', 'Impuestos', )
...
class product_template(models.Model):
    ...
    shop_id = fields.Many2one('sale.shop', string='Hotel',
                              help="Mostrará la lista de tiendas que pertenecen a las empresas permitidas del usuario registrado. \Asignar una tienda hará que el producto sea exclusivo para la tienda seleccionada.")
    product_nature = fields.Selection(
        [('kot', 'KOT'), ('bot', 'BOT')], 'Naturaleza del producto')
...
class hotel_restaurant_kitchen_order_tickets(models.Model):
    ...
    shop_id = fields.Many2one('sale.shop', 'Hotel', required=True)
    pricelist_id = fields.Many2one('product.pricelist', 'Tarifa de precios', )
...
class hotel_menucard(models.Model):
    ...
    company_id = fields.Many2one(
        'res.company', string='Compañía', default=lambda *a: False)
...
class hotel_resv_id_details(models.Model):
    ...
    client_id = fields.Many2one("id.master", "Tipo de documento")
    partner_name = fields.Char('Nombre del huésped')
    issuing_auth = fields.Char('Autoridad emisora')
    gender = fields.Selection(
        [('M', 'Male'), ('F', 'Female')], 'Género')
    country_id = fields.Many2one("res.country", "País")
    date_birth = fields.Date('Fecha de nacimiento')
    valid_from = fields.Date('Válido desde')
    valid_to = fields.Date('Válido hasta')
    reservation_id = fields.Many2one('hotel.reservation', 'ID de reservación')
    folio_id = fields.Many2one('hotel.folio', 'ID de Folio')
