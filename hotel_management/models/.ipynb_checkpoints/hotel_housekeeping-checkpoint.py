...
class rr_housekeeping(models.Model):
    ...
    name = fields.Char(
        'Número de solicitud', readonly=True, states={'draft': [('readonly', False)]})
    date = fields.Datetime('Fecha de solicitud', required=True, readonly=True, default=lambda *
                           a: time.strftime('%Y-%m-%d %H:%M:%S'), states={'draft': [('readonly', False)]})
    activity = fields.Selection([('repair', 'Reparación'), ('replaced', 'Reemplazo')], 'Actividad',
                                index=True, required=True, readonly=True, states={'draft': [('readonly', False)]})
    requested_by = fields.Many2one('res.users', 'Solicitado por el usuario', readonly=True, states={'draft': [('readonly', False)]})
    requested_by_partner = fields.Many2one('res.partner', 'Solicitado por', readonly=True, states={'draft': [('readonly', False)]})
    source = fields.Selection([('intern', 'Observaciones internas'), ('guest', 'Huésped')], 'Origen',
                              default='intern', required=True, readonly=True, states={'draft': [('readonly', False)]})
    assign_to = fields.Selection([('intern', 'Interno'), ('third_party', 'Servicio de terceros')], 'Método de asignación', default='intern',
                                 required=True, readonly=True, states={'draft': [('readonly', False)], 'confirmed': [('readonly', False)]})
    assigned_third_party = fields.Many2one('res.partner', 'Asignar a un tercero', readonly=True, states={
                                           'draft': [('readonly', False)], 'confirmed': [('readonly', False)]})
    assigned_internal = fields.Many2one('res.users', 'Asiganado a', readonly=True, states={
                                        'draft': [('readonly', False)], 'confirmed': [('readonly', False)]})
    room_no = fields.Many2one('hotel.room', 'Número de habitación', required=True, readonly=True, states={
                              'draft': [('readonly', False)]})
    approved_by = fields.Char('Aprobado por')
    rr_line_ids = fields.One2many('rr.housekeeping.line', 'rr_line_id', 'Información de reparación / reemplazo',
                                  required=True, readonly=True, states={'draft': [('readonly', False)], 'confirmed': [('readonly', False)]})
    state = fields.Selection([('draft', 'Borrador'), ('confirmed', 'Confirmado'), ('assign', 'Asignado'), (
        'done', 'Realizado'), ('cancel', 'Cancelado')], 'State', default="draft", readonly=True, index=True)
    complaint = fields.Char('Queja / Observación', readonly=True, states={'draft': [('readonly', False)]})
    shop_id = fields.Many2one('sale.shop', 'Hotel', required=True, readonly=True, states={
                              'draft': [('readonly', False)]})
    company_id = fields.Many2one('res.company', related='shop_id.company_id', string='Compañía', store=True)
...
class rr_housekeeping_line(models.Model):
    ...
    product_line_ids = fields.One2many(
        'product.product.line', 'product_line_id', 'Detalles de producto')
    qty = fields.Float('Cantidad', default=1)
    uom = fields.Many2one('uom.uom', 'UdM')
    source_locatiion = fields.Many2one('stock.location', 'Ubicación de origen')
    dest_locatiion = fields.Many2one('stock.location', 'Ubicación de destino')
    info_id = fields.Many2one('issue.material.details', 'ID del Material')
...
class issue_material_details(models.Model):
    ...
    name = fields.Char('Documento de entrega')
    request_id = fields.Many2one('rr.housekeeping', 'Número de solicitud',
                                 required=True, readonly=True, states={'draft': [('readonly', False)]})
    repair_ids = fields.One2many('rr.housekeeping.line', 'info_id', 'Información sobre sustitución de productos', readonly=True, states={
                                 'draft': [('readonly', False)]})
    complaint = fields.Char(
        'Queja / Observación', readonly=True, states={'draft': [('readonly', False)]})
    shop_id = fields.Many2one('sale.shop', 'Hotel', required=True, readonly=True, states={
                              'draft': [('readonly', False)]})
    state = fields.Selection([('draft', 'Borrador'), ('confirm', 'Confirmado'), (
        'done', 'Realizado')], 'State', default='draft', readonly=True, index=True)
#     company_id = fields.related('shop_id','company_id',type='many2one',relation='res.company',string='Company',store=True)
    company_id = fields.Many2one(
        'res.company', related='shop_id.company_id', string='Compañia', store=True)
