...
class check(http.Controller):
            ...
            if 'no_room' in rtype and 'chk' in rtype:
                if rtype['chk'] == 'on':
                    for lno in range(0, (int(rtype['no_room']))):
                        ...
                        if res1[lno].taxes_id.id:room_line_id.update({'taxes_id': [(6, 0, [res1[lno].taxes_id.id])]})
    ...
    def confirm_order(self, **post):
        ...
        return request.redirect("/shop/payment2")
    ...
    def cart_update(self, product_id, add_qty=1, set_qty=0, **kw):
        product_id = request.env['product.template'].sudo().search([('id', '=', product_id)])
        _logger.info("PRECIO DEL PRODUCTO====>>>>%s", product_id.list_price)
        vals = {
            ...
            'tax_id': product_id.taxes_id.id,
        }
        
        _logger.info("RESERVATION ID====>>>>%s",request.session.get('reservation_order_id'))
        if request.session.get('reservation_order_id'):
            order = request.env['hotel.reservation'].sudo().search(
