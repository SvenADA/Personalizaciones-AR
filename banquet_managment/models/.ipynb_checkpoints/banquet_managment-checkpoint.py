...
class BanquetQuotation(models.Model):
    ...
    number_of_days = fields.Integer(compute='count_total_days', string="Número de días",
                                    default=1,
...
class HotelReservationLine(models.Model):
    ...
    cost_price_subtotal = fields.Float(
        compute='_amount_line_cost', string='Cost Subtotal', digits='Precio de coste')
    pur_tax_ids = fields.Many2many(
        'account.tax', 'pur_hotel_line_tax_rel', 'hotel_line_id', 'tax_id', 'Impuestos de compra')
    purches_bol = fields.Boolean('Show Purchase Tax')
    number_of_days = fields.Integer(
        compute='count_total_days', string="Número de días", default=1)
...
class HotelReservation(models.Model):
    ...
    deposit_policy = fields.Selection([('percentage', 'Porcentaje de depósito'), ('no_deposit', 'Sin depósito')],
                                      'Política de depósitos', required=True, readonly=True,
                                      states={'draft': [('readonly', False)]}, default='no_deposit')
    percentage = fields.Float("Porcentaje/Importe del depósito")
    min_dep_amount = fields.Float("Importe mínimo del depósito")
    deposit_recv_acc = fields.Many2one('account.account', string="Cuenta de depósito", required=False,
    ...
    total_cost1 = fields.Float(
        compute='_get_total_rental_cost', string="Coste total de reservación", )
    deposit_cost = fields.Float(
        compute='_get_deposit_cost1', string="Coste de depósito", )
    banquet_id = fields.Many2one(
        'banquet.quotation', 'Referencia de reservación', readonly=True)
